...
--힌트
병합 정렬은 표준 라이브러리에서 정렬을 구현할 때 퀵 정렬이나 힙 정렬의 대안으로 사용하는 최적화된 
정렬 알고리즘입니다. 병합 정렬은 다음과 같은 알고리즘을 사용합니다.
...
//vscode상에서는 문제없는데 테스트코드 돌리면 에러발생. 원인을 알수가 없다
const mergeSort = function (arr) {
  let mergeArr = arr.map((v) => [v]);

  while (mergeArr.length > 1) {
    let nextMerge = [];
    for (let i = 0; i < mergeArr.length; i += 2) {
      if (!mergeArr[i + 1]) {
        nextMerge.push(mergeArr[i]);
        break;
      }
      ...
      let left = 0,
        right = 0;
      while (mergeArr[i].length > left || mergeArr[i + 1].length > right) {
        if (
          (mergeArr[i][left] || Infinity) < (mergeArr[i + 1][right] || Infinity)
        )
          merge.push(mergeArr[i][left++]);
        else merge.push(mergeArr[i + 1][right++]);
      }

      nextMerge.push(merge);
    }

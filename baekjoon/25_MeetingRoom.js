...
한 회의가 끝나는 것과 동시에 다음 회의가 시작될 수 있다. 회의의 시작시간과 끝나는 시간이 같을 
...
const timeTable = input.slice(1).sort((a, b) => {
  //끝나는 시간이 빠른 순으로 오름차순 정렬
  //끝나는 시간이 같을경우 시작시간이 빠른순으로 오름차순
...
const result = [timeTable[0]];

for (let i = 1; i < timeTable.length; i++) {
  const last = result[result.length - 1].split(" ")[1];
  const curr = timeTable[i].split(" ")[0];
  if (Number(last) <= Number(curr)) result.push(timeTable[i]);
}

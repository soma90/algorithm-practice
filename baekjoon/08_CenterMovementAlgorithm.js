...
if (input === 1) return console.log(9);

//1일떄 점 개수와 생성되는 사각형 초기값
let result = 9;
let squares = 4;

/*  
한번 반복될때 마다 사각형은 4배씩 증가한다.
한개의 사각형마다 5개의 점이 추가로 생성되고 중복되는 점들을 계산해서 제외해준다.
*/
for (let i = 2; i <= input; i++) {
  //result += squares * 5 - (Math.sqrt(squares) - 1) * Math.sqrt(squares) * 2;
  result += squares * 5 - (squares - Math.sqrt(squares)) * 2;
  squares *= 4;
}

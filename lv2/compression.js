...
function solution(msg) {
  ...
  for (let i = 1; i <= msg.length; ) {
    const w = msg.slice(start, i);
    let output = lzwMap.get(w);
    //console.log(w, output);

    if (!output) {
      lzwMap.set(w, n++);
      answer.push(outputStack.pop());
      outputStack = [];
      wStack = [];
      start = i - 1;
    } else {
  ...
  if (outputStack.length !== 0) answer.push(outputStack.pop());

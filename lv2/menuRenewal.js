...
function solution(orders, course) {
  ...
  //메뉴를 받아 만들 수 있는 메뉴조합 구하기
  const getCombination = (prefix, arr, n) => {
    if (n === 1) return arr.map((el) => prefix + el);
    let result = [];
    for (let i = 0; i < arr.length; i++) {
      const menu = getCombination(prefix + arr[i], arr.slice(i + 1), n - 1);
      result.push(...menu);
    }
    return result.map((el) => el.split("").sort().join(""));
  };
  ...
  for (let i = 0; i < course.length; i++) {
    ...
    for (let j = 0; j < orders.length; j++) {
      getCombination("", orders[j].split(""), course[i]).forEach((el) => {
        const num = (menuMap.get(el) || 0) + 1;
        if (num > max) max = num;
        menuMap.set(el, num);
      });
    }
    //가장 많이 나온 메뉴 저장
    for (let [key, value] of menuMap.entries()) {
      if (value === max && max >= 2) answer.push(key);
    }
  }

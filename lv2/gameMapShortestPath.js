...
/* function solution(maps) {
  let answer = -1;
  let [goalx, goaly] = [maps.length - 1, maps[0].length - 1];
  let visited = [[0, 0]];
  ...
  const dfs = (x, y) => {
    ...
    if (x === goalx && y === goaly) {
      ...
      if (answer === -1 || answer > visited.length) answer = visited.length;
      return;
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (
        maps[nx] &&
        maps[nx][ny] === 1 &&
        !visited.some((el) => el[0] === nx && el[1] === ny)
      ) {
        visited.push([nx, ny]);
        dfs(nx, ny);
        visited.pop();
...
function solution(maps) {
  ...
  while (needVisit.length > 0) {
    ...
    for (let i = 0; i < 4; i++) {
      let [nx, ny] = [x + dx[i], y + dy[i]];
      if (maps[nx] && maps[nx][ny] === 1 && !visited[nx][ny]) {
        visited[nx][ny] = true;
        needVisit.push([nx, ny, c + 1]);
      }
    }
  }
  //console.log(visited);

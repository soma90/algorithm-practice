...
예를 들면, "abcabcdede"와 같은 문자열은 전혀 압축되지 않습니다. "어피치"는 이러한 단점을 해결하기 위해 문자열을 1개 
이상의 단위로 잘라서 압축하여 더 짧은 문자열로 표현할 수 있는지 방법을 찾아보려고 합니다.
...
function solution(s) {
  ...
  for (let i = 1; i < s.length; i++) {
    ...
    for (let j = 0; ; j += i) {
      ...
      let prev = stack.at(-1);
      if (sliced === prev) stack.push(sliced);
      else {
        if (stack.length > 1) str += stack.length + prev;
        else str += prev;        
        stack = [sliced];
      }
      if (sliced === "") break;

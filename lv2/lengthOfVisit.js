...
function solution(dirs) {
  ...
  for (const dir of dirs) {
    const [nextRow, nextCol] = [row + move.get(dir).dr, col + move.get(dir).dc];
    //좌표범위 넘어가는지 확인
    if (Math.abs(nextRow) > 5 || Math.abs(nextCol) > 5) continue;
    //방문한길인지 확인 후 방문처리
    const posStr = `${row},${col}`;
    const nextPosStr = `${nextRow},${nextCol}`;
    if (
      !visited.get(posStr)?.includes(nextPosStr) &&
      !visited.get(nextPosStr)?.includes(posStr)
    ) {
      answer++;
      visited.set(posStr, [...(visited.get(posStr) || []), nextPosStr]);
    }
    //다음좌표 설정

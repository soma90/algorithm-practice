...
function solution(fees, records) {
  ...
  records.forEach((record) => {
    ...
    let convertTime = hours * 60 + minutes;
    //입출차 기록 맵에 저장
    if (status === "IN") {
      recordsMap.set(Number(num), [
        ...(recordsMap.get(Number(num)) || []),
        [convertTime, 23 * 60 + 59],
      ]);
    } else if (status === "OUT") {
      let recordData = recordsMap.get(Number(num));
      recordData[recordData.length - 1][1] = convertTime;
      recordsMap.set(Number(num), recordData);
    }
  });
  ...
  [...recordsMap.keys()]
    ...
    .forEach((num) => {
      //주차 누적시간 구하기
      let time = 0;
      recordsMap.get(num).forEach(([start, end]) => (time += end - start));
      //요금 계산
      let extra = 0;
      if (time > fees[0])
        extra = Math.ceil((time - fees[0]) / fees[2]) * fees[3];

      answer.push(fees[1] + extra);
    });

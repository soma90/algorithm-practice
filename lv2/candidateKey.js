...
--문제 설명
프렌즈대학교 컴퓨터공학과 조교인 제이지는 네오 학과장님의 지시로, 학생들의 인적사항을 정리하는 
업무를 담당하게 되었다.
...
function solution(relation) {
  ...
  //후보키 최소성 체크
  const isSubset = (visited) => {
    if (!cands.some((row) => row.every((x) => visited.includes(x)))) {
      return false;
    }
    return true;
  };

  const dfs = (maxLoop, nextIndex = 0, visited = [], curLoop = 0) => {
    //후보키 최소성 체크
    if (curLoop === maxLoop && !isSubset(visited)) {
      //후보키 조합 만들기
      let candList = [];
      for (let i = 0; i < relation.length; i++) {
        let cand = visited.map((el) => relation[i][el]);
        candList.push(cand);
      }
      //후보키 유일성 체크
      if (!hasDuplicateRows(candList)) {
        cands.push([...visited]);
      }
      //console.log(visited, candList);
      return;
    }

    for (let i = nextIndex; i < relation[0].length; i++) {
      visited.push(i);
      dfs(maxLoop, i + 1, visited, curLoop + 1);
      visited.pop();
    }
  };

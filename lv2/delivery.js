...
합니다. 도로를 지날 때 걸리는 시간은 도로별로 다릅니다. 현재 1번 마을에 있는 음식점에서 각 마을로 음식 배달을 
하려고 합니다. 각 마을로부터 음식 주문을 받으려고 하는데, N개의 마을 중에서 K 시간 이하로 배달이 가능한 
마을에서만 주문을 받으려고 합니다. 다음은 N = 5, K = 3인 경우의 예시입니다.

배달_1_uxun8t.png

위 그림에서 1번 마을에 있는 음식점은 [1, 2, 4, 5] 번 마을까지는 3 이하의 시간에 배달할 수 있습니다. 그러나 
3번 마을까지는 3시간 이내로 배달할 수 있는 경로가 없으므로 3번 마을에서는 주문을 받지 않습니다. 
...
a, b(1 ≤ a, b ≤ N, a != b)는 도로가 연결하는 두 마을의 번호이며, c(1 ≤ c ≤ 10,000, c는 자연수)는 도로를 
지나는데 걸리는 시간입니다.
...
function solution(N, road, K) {
  ...
  //road정보 map으로 저장
  for (const r of road) {
    roadMap.set(r[0], [...(roadMap.get(r[0]) || []), [r[1], r[2]]]);
    roadMap.set(r[1], [...(roadMap.get(r[1]) || []), [r[0], r[2]]]);
  }
  //시작정보 heap에 넣기
  heap.insert([1, 0]);
  dists[1] = 0;
  ...
  //다익스트라를 이용해 각지점 거리 저장
  while (heap.items.length > 0) {
    const [point, dist] = heap.popMin();
    if (dists[point] < dist) continue;
    dists[point] = dist;

    for (let [nextPoint, nextDist] of roadMap.get(point)) {
      if (visited[nextPoint].includes(nextDist)) continue;
      visited[nextPoint].push(nextDist);
      heap.insert([nextPoint, dist + nextDist]);
    }
  }

  //거리가 3이하 확인
  return dists.filter((el) => el <= K).length;
}

...
function solution(n, wires) {
  ...
  const dfs = (graph, node, visited = []) => {
    ...
    neighbors.forEach((neighbor) => dfs(graph, neighbor, visited));
    return visited;
  };
  //간선을 하나씩 제거 하면서 최소값 찾기
  for (let i = 0; i < wires.length; i++) {
    let cpyWires = [...wires];
    let [wire] = cpyWires.splice(i, 1);

    let result = Math.abs(
      dfs(cpyWires, wire[0]).length - dfs(cpyWires, wire[1]).length
    );
    //console.log(wire[0], dfs(cpyWires, wire[0]), dfs(cpyWires, wire[1]));    

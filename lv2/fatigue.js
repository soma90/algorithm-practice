...
/**
 * -- 풀이 1
 * 완전 탐색을 이용해 탐험 할 수 있는 최대 던전 수를 구합니다.
 * 각 던전의 최소 피로도와 소모 피로도와 현재 피로도와 비교해서 가능한 던전일 경우 재귀를 이용해 탐색합니다.
 * 재귀를 호출할때마다 탐색한 던전수를 증가시키고 탐색한 던전수의 최대값을 업데이트 합니다.
 */
function solution(k, dungeons) {
  let answer = 0;
  const visited = Array.from({ length: dungeons.length }, () => false);

  const permute = (currentK, count) => {
    answer = Math.max(answer, count);

    for (let i = 0; i < dungeons.length; i++) {
      const minK = currentK - dungeons[i][0];
      const nextK = currentK - dungeons[i][1];
      if (visited[i] || minK < 0 || nextK < 0) continue;

      visited[i] = true;
      permute(nextK, count + 1);
      visited[i] = false;
    }
  };
...
function solution2(k, dungeons) {
  ...
  const explore = (_k, _d, _c = 0) => {
    _d.forEach((el, i) => {
      if (el[0] <= _k) {
        explore(_k - el[1], [..._d.slice(0, i), ..._d.slice(i + 1)], _c + 1);
      }
    });
    answer.push(_c);
  };

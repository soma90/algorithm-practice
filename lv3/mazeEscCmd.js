...
격자의 바깥으로는 나갈 수 없습니다.
(x, y)에서 (r, c)까지 이동하는 거리가 총 k여야 합니다. 이때, (x, y)와 (r, c)격자를 포함해, 
...
격자의 크기를 뜻하는 정수 n, m, 출발 위치를 뜻하는 정수 x, y, 탈출 지점을 뜻하는 정수 r, c, 탈출까지 
이동해야 하는 거리를 뜻하는 정수 k가 매개변수로 주어집니다. 이때, 미로를 탈출하기 위한 경로를 
return 하도록 solution 함수를 완성해주세요. 단, 위 조건대로 미로를 탈출할 수 없는 경우 "impossible"을 
return 해야 합니다.
...
탈출까지 이동해야 하는 거리 k가 4입니다. 이때, 이동 거리가 4이면서, S에서 E까지 이동할 수 있는 경로는 
존재하지 않습니다.
...
function solution(n, m, x, y, r, c, k) {
  ...
  while (stack.length > 0) {
    const [cx, cy, move, count] = stack.pop();
    const dist = Math.abs(r - cx) + Math.abs(c - cy);
    const remainCount = k - count;

    if (count === k) {
      if (cx === r && cy === c) return move;
      continue;
    }

    if (dist > remainCount || (remainCount - dist) % 2 === 1) continue;

    for (let i = 3; i >= 0; i--) {
      const nx = cx + dx[i];
      const ny = cy + dy[i];
      if (nx <= n && nx > 0 && ny <= m && ny > 0) {
        stack.push([nx, ny, move + cmd[i], count + 1]);
      }
    }
  }
...
let result = solution(3, 4, 2, 3, 3, 1, 5); 
console.log(result);

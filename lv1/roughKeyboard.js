...
function solution(keymap, targets) {
  return (answer = targets.map((el) => {
    //target의 각 문자 체크
    let sum = 0;
    for (let i = 0; i < el.length; i++) {
      //keymap에서 가능한 인덱스 체크
      const char = el[i]; //체크할 문자
      let index = Infinity; //key맵에 해당되는 index
      for (let j = 0; j < keymap.length; j++) {
        const findIndex = keymap[j].indexOf(char);
        //문자열 작성할수 없을때
        if (findIndex !== -1) {
          index = Math.min(index, findIndex + 1);
        }
        //console.log(findIndex, char)
      }

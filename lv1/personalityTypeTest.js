...
function solution(survey, choices) {
  ...
  let personalTpye = ["R", "T", "C", "F", "J", "M", "A", "N"];

  //점수저장
  for (let i = 0; i < choices.length; i++) {
    if (choices[i] < 4) {
      const score = 4 - choices[i];
      scoreMap.set(survey[i][0], (scoreMap.get(survey[i][0]) || 0) + score);
    } else if (choices[i] > 4) {
      const score = choices[i] - 4;
      scoreMap.set(survey[i][1], (scoreMap.get(survey[i][1]) || 0) + score);
    }
  }

  //저장된 점수에서 성격유형 체크
  for (let i = 0; i < personalTpye.length; i += 2) {
    const score1 = scoreMap.get(personalTpye[i]) || 0;
    const score2 = scoreMap.get(personalTpye[i + 1]) || 0;
    if (score1 > score2) answer += personalTpye[i];
    else if (score1 < score2) answer += personalTpye[i + 1];
    else
      answer +=
        personalTpye[i] > personalTpye[i + 1]
          ? personalTpye[i + 1]
          : personalTpye[i];
  }
...
console.log(result); //"TCMA"
result = solution(["TR", "RT", "TR"], [7, 1, 3]);
console.log(result); //"RCJA"
result = solution(["RT", "RT", "RT"], [7, 2, 4]);
console.log(result); //"TCJA"

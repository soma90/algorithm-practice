...
"apeach"	0
"neo"	2
위 예시에서는 2번 이상 신고당한 "frodo"와 "neo"의 게시판 이용이 정지됩니다. 이때, 각 유저별로 신고한 아이디와 
정지된 아이디를 정리하면 다음과 같습니다.
...
function solution(id_list, report, k) {
  ...
  let reportFrom = new Map(); //key아이디를 신고한 아이디

  //신고한 아이디 저장
  for (let i = 0; i < report.length; i++) {
    const p = report[i].split(" ");
    //key아이디를 신고한 아이디 저장
    let fromArr = reportFrom.get(p[1]) || [];
    fromArr = [...fromArr, p[0]];
    reportFrom.set(p[1], [...new Set(fromArr)]);
  }

  //유저별 메일 횟수 저장
  for (let [_, val] of reportFrom.entries()) {
    if (val.length < k) continue;
    for (let j = 0; j < id_list.length; j++) {
      if (val.includes(id_list[j])) answer[j]++;
    }
  }

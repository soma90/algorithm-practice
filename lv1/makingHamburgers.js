...
/* function solution(ingredient) {
  ...
  while (ingredientStr.length > 0) {
    const index = ingredientStr.indexOf(target);
    if (index !== -1) {
      ingredientStr =
        ingredientStr.substring(0, index) +
        ingredientStr.substring(index + target.length);
      answer++;
    } else break;
  }
...
/* function solution(ingredient) {
  ...
  for (let i = 0; i < ingredient.length; i++) {
    let currIng = ingredient[i];
    if(currIng === 1) {
      if(make[make.length - 1] === 3) {
        make.pop();
        answer++;
      } else {
        make.push(1)
      ...
      if(make[make.length - 1] + 1 === currIng) {
        make[make.length - 1] = currIng
      }
    }
    console.log(make)
  }
...
function solution(ingredient) {
  ...
  for (let i = 0; i < ingredient.length; i++) {
    make.push(ingredient[i]);
    if (ingredient[i] === 1 && make.slice(-4).join("") === "1231") {
      answer++;
      make.splice(-4);
    }
  }
...
let result = solution([1, 1, 3, 2, 3, 1]);

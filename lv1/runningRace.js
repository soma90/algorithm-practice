...
function solution(players, callings) {
  ...
  for (let i = 0; i < callings.length; i++) {
    ...
    const curRank = playersMap.get(curPlayer);
    const prevRank = curRank - 1;
    const prevPlayer = rankMap.get(prevRank);

    playersMap.set(curPlayer, prevRank);
    playersMap.set(prevPlayer, curRank);
    rankMap.set(curRank, prevPlayer);
    rankMap.set(prevRank, curPlayer);
  }

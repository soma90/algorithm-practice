...
function solution(today, terms, privacies) {
  ...
  //계약서당 만료개월 저장
  const termsDate = new Map();
  for (let i = 0; i < terms.length; i++) {
    const term = terms[i].split(" ");
    termsDate.set(term[0], Number(term[1]));
  }

  //만료날짜를 비교해서 삭제할 개인정보 저장
  for (let i = 0; i < privacies.length; i++) {
    const privacy = privacies[i].split(" ");
    const dateToday = new Date(today);
    const date = new Date(privacy[0]);
    date.setMonth(date.getMonth() + termsDate.get(privacy[1]));
    if(dateToday.getTime() >= date.getTime()) answer.push(i + 1);
  }
